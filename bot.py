...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "📄 Файлы: загрузите PDF/DOCX/TXT/Excel — извлеку текст или подготовлю изменённый файл.\n"
        "🌐 Онлайн-поиск: /search запрос или фразы вроде «проверь в интернете».\n"
        "👥 В группах: отвечаю только на @упоминание, ответ на моё сообщение или команду."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_message.strip():
        await update.message.reply_text("Я на связи. Напишите вопрос после упоминания бота.")
        return
    ...
    if is_group_chat(update):
        ...
        prompt = (
            ...
            f"{user_message}\n\n"
            "Важно: ты видишь только сообщения, адресованные тебе, и записанные договорённости, "
            "не делай вид, что прочитала всю переписку чата."
        )

    if should_use_online_search(user_message):
        response = await asyncio.to_thread(answer_online, user_message, str(chat_id))
    else:
        response = await asyncio.to_thread(ask_deepseek, prompt, str(chat_id))

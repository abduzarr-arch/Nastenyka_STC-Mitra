...
def init_db():
    ...
    ''')
    _ensure_column(c, "conversation", "dialog_key", "TEXT")
    c.execute("CREATE INDEX IF NOT EXISTS idx_conversation_dialog_key ON conversation(dialog_key, timestamp)")
...
def _legacy_numeric_user_id(dialog_key):
    if isinstance(dialog_key, int):
        return dialog_key
    text = str(dialog_key or "")
...
def add_to_conversation(user_id, role, content):
    dialog_key = str(user_id)
    conn = _connect()
    c = conn.cursor()
    c.execute('''
        INSERT INTO conversation (user_id, dialog_key, role, content)
        VALUES (?, ?, ?, ?)
    ''', (_legacy_numeric_user_id(dialog_key), dialog_key, role, content))
    conn.commit()
    conn.close()

...
def answer_online(query: str, user_id: str) -> str:
    ...
    try:
        if TAVILY_API_KEY:
            ...
            if OPENAI_API_KEY:
                logger.warning("TAVILY_API_KEY is set, but DEEPSEEK_API_KEY is missing. Falling back to OpenAI web search.")
                return answer_with_openai_web_search(query, user_id)
            return (
                "TAVILY_API_KEY задан, но для режима Tavily нужен еще DEEPSEEK_API_KEY, "
                "чтобы сформировать ответ по найденным источникам. Либо добавьте OPENAI_API_KEY "
                "для запасного web search."
            )
        if OPENAI_API_KEY:
            return answer_with_openai_web_search(query, user_id)
        return (
...
async def search_command(update, context) -> None:
    ...
    answer = await asyncio.to_thread(answer_online, query, get_dialog_key(update))

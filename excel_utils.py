...
def remember_excel_file(context, chat_id: int, source_path: str, file_name: str) -> Dict[str, Any]:
    """Сохраняет присланный/созданный Excel, чтобы следующий текст пользователя мог его изменить."""
    old = context.user_data.get("last_excel") if hasattr(context, "user_data") else None
    ...
    context.user_data["last_excel"] = info
    context.user_data["awaiting_excel_request"] = True
    return info


def _get_recent_excel_context(context, max_age_seconds: int = 24 * 60 * 60) -> Optional[Dict[str, Any]]:
    info = context.user_data.get("last_excel") if hasattr(context, "user_data") else None
    if not info:
        return None
    path = info.get("path")
    if not path or not os.path.exists(path):
        context.user_data.pop("last_excel", None)
        context.user_data.pop("awaiting_excel_request", None)
        return None
...
def _is_excel_followup_request(context, text: str) -> bool:
    if not text:
        return False
    lowered = text.lower()
    if context.user_data.get("awaiting_excel_request"):
        return True
...
async def handle_excel_document(update, context) -> None:
    ...
    try:
        ...
        remember_excel_file(context, update.effective_chat.id, tmp_path, file_name)
        ...
        if looks_like_excel_edit_request(user_request):
            ...
            if out_path:
                ...
                remember_excel_file(context, update.effective_chat.id, out_path, os.path.basename(out_path))
                text = message
                if changes:
...
async def handle_excel_followup_text(update, context, text: str) -> bool:
    ...
    info = _get_recent_excel_context(context)
    if not info or not _is_excel_followup_request(context, text):
        return False
    ...
    try:
        ...
        if looks_like_excel_edit_request(request_text):
            ...
            await update.message.reply_text(answer)
        context.user_data["awaiting_excel_request"] = False
        return True
    except Exception as e:

...
def is_create_excel_request(text: str) -> bool:
    if not text:
        return False
    lowered = text.lower()
    has_excel_word = any(word in lowered for word in ("excel", "xlsx", "эксель", "ексель", "таблицу", "таблица"))
    has_create_word = any(word in lowered for word in ("создай", "сделай", "сформируй", "подготовь", "собери"))
    # Чтобы не перехватывать обычные фразы вроде «сделай выводы по таблице» без просьбы создать файл.
    wants_file = any(word in lowered for word in ("файл", "excel", "xlsx", "эксель", "ексель"))
    return has_excel_word and has_create_word and wants_file
    ...
    try:
        ...
        if not user_request:
            ...
            await update.message.reply_text(
                ...
                "Теперь можно написать вопрос или отправить файл ещё раз с подписью, например:\n"
        ...
        if looks_like_excel_edit_request(user_request):
            out_path, message, changes = edit_excel_with_ai(tmp_path, file_name, user_request, user_id)
            if out_path:
                text = message
                if changes:
            ...
            answer = analyze_excel_with_ai(tmp_path, file_name, user_request, user_id)
